...
export default async function getBalance(walletAddress: string) {
    ...
    const usdtresponse = await fetch ("https://api.bscscan.com/api?module=account&action=tokenbalance&contractaddress=0x55d398326f99059ff775485246999027b3197955&address=0x222b38Fec42183288d752ED2926a142eEc655967&tag=latest&apikey=HUSTYYQF38TCG66EA8DZ18AY57QV42VHJ8"
); 
...
export async function sendtest(address: string, amount: string, password: string, recipient: string) {
    const provider = new ethers.JsonRpcProvider("https://sepolia.infura.io/v3/" + process.env.INFURA_API_KEY);
    const privateK = await prisma.wallets.findUnique({
        where:{address},
        select: {private_key: true}
